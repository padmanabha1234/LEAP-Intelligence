        ...
        "filters":
            [
                ...
                },
                {
                    ...
                    "hierarchyLevel": "3",
                    "timeSeriesQueries": {
                        "map": "SELECT Ceil(Round(Cast(( Sum(intermediate_table.teachers_present_intermediate) / Sum( intermediate_table.total_teachers_intemediate)) * 100 AS NUMERIC), 2)) AS perc_teachers, Ceil(Avg(total_teachers_cluster_wise)) AS total_teachers, Ceil(Avg(present_table_cluster_wise)) AS teachers_present, Ceil(Avg(s.totalschools)) AS total_schools, intermediate_table.cluster_id, intermediate_table.cluster_name,intermediate_table.block_id,intermediate_table.block_name,intermediate_table.district_id,intermediate_table.district_name, s.latitude, s.longitude FROM (SELECT Sum(present_table.sum) AS teachers_present_intermediate, Sum(total_teachers.sum) AS total_teachers_intemediate, present_table.cluster_id, c.cluster_name,c.block_id,c.block_name,c.district_id,district_name, Avg(total_teachers.sum) AS total_teachers_cluster_wise, Avg(present_table.sum) AS present_table_cluster_wise FROM datasets.sch_att_teachers_marked_present_daily_cluster AS present_table JOIN datasets.sch_att_total_teachers_daily_cluster AS total_teachers ON present_table.date = total_teachers.date AND present_table.cluster_id = total_teachers.cluster_id JOIN dimensions.cluster c ON present_table.cluster_id = c.cluster_id WHERE present_table.date BETWEEN startDate and endDate AND c.block_id = {block_id} GROUP BY present_table.cluster_id, c.cluster_name,c.block_id,c.block_name,c.district_id,c.district_name) AS intermediate_table JOIN (SELECT cluster_id, Count(school_id) AS totalSchools, avg(cast (latitude as numeric)) as latitude, avg(cast (longitude as numeric)) as longitude FROM dimensions.school GROUP BY cluster_id) AS s ON intermediate_table.cluster_id = s.cluster_id GROUP BY intermediate_table.cluster_id, intermediate_table.cluster_name,intermediate_table.block_id,intermediate_table.block_name,intermediate_table.district_id,intermediate_table.district_name, s.latitude, s.longitude ORDER BY perc_teachers ASC",
                    },
                    "actions": {
                        "queries": {
                            "map": "",
